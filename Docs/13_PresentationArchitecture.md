...
BrigadePresentationState содержит:

- BrigadeId;
- номер и тип бригады;
- текущую и максимальную численность;
- опыт и максимальный достигнутый опыт;
- мораль;
- усталость;
- прозвище;
- ветеранскую традицию;
- признак расформирования;
- текущий ModuleId;
- признаки присутствия и работы.

Собственное состояние Brigade и её оперативное
расположение захватываются в один неизменяемый
Presentation-снимок.

Изменение численности, морали, усталости или работы
после захвата не изменяет уже переданный снимок.

Бригады упорядочиваются детерминированно:

1. Number;
2. BrigadeId.

Отдельные будущие визуальные представители не являются
самостоятельными объектами Simulation.

## 12. BrigadeView

Для каждой развёрнутой бригады создаётся BrigadeView.

BrigadeView:

- получает BrigadePresentationState;
- получает ModulePresentationState текущего отсека;
- не хранит Brigade;
- не выполняет игровые расчёты;
- может быть повторно использован при переходе бригады
  в другой отсек;
- визуально различает Idle и Working.

Несколько бригад внутри одного модуля получают
детерминированные технические слоты.
